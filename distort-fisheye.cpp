...
int main(int argc, char* argv[])
{
    ...
    string fn_param = "fisheye-params.json";
    if (argc > 1)
        fn_image = argv[1];
    if (argc > 2)
        fn_param = argv[2];
    ...
    Mat R;
    Mat distorted_cam_mtx = cam_mtx.clone();

    // add distortion
    Mat map_x = Mat(image_size, CV_32FC1);
    Mat map_y = Mat(image_size, CV_32FC1);
    vector<Point2f> distorted_output_pts, undistorted_input_pts;
    for (int y = 0; y < image_size.height; ++y)
        for (int x = 0; x < image_size.width; ++x)
            undistorted_input_pts.emplace_back(x, y);
    fisheye::undistortPoints(undistorted_input_pts, distorted_output_pts, cam_mtx, dis_cef, R, distorted_cam_mtx);
    for (int y = 0; y < image_size.height; ++y) {
        float* ptr1 = map_x.ptr<float>(y);
        float* ptr2 = map_y.ptr<float>(y);
        for (int x = 0; x < image_size.width; ++x) {
            const auto& pt = distorted_output_pts[y * image_size.width + x];
            ptr1[x] = pt.x;
            ptr2[x] = pt.y;
        }
    }
